...
export default function ConPercepcion() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [preProv, setPreProv] = useState("");
  const [utilidad, setUtilidad] = useState("");

  const pre = parseFloat(preProv) || 0;
  const util = parseFloat(utilidad) || 0;

  const perc = pre > 0 ? pre / 1.01 / 100 : 0;
  const pt1 = pre - perc;
  const bi1 = pt1 / 1.18;
  const igv1 = pt1 - bi1;
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>CON PERCEPCION</Text>
        </View>
        ...
        {menuOpen && (
          <View style={styles.menu}>
            ...
            <TouchableOpacity
              onPress={() => {
                setMenuOpen(false);
                router.push("/(tabs)/comercial/sin-percepcion");
              }}
            >
        ...
        <View style={styles.grid}>
          <View style={styles.card}>
            ...
            <Input label="PRE PROV." value={preProv} onChangeText={setPreProv} />
            <Result label="PERC." value={format(perc)} />
            <Result label="P.T." value={format(pt1)} />
